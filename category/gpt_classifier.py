...
OUTPUT_CSV  = r"C:\Users\MICHA\Codes\PromptDescriptionPrj\category\prompt_category_gpt_4.csv"
...
def process_and_save_prompts(client: OpenAI, prompts: List[Dict], output_path: str):
    ...
    with open(output_path, 'w', newline="", encoding='utf-8') as f:
        ...
        for i, p in enumerate(prompts):
            print(f"[{i+1}/{len(prompts)}] ID {p['id']}: {p['prompt'][:60]}...", end=" ", flush=True)
            try:
...
def interactive_mode(client: OpenAI):
    ...
    print("   Type 'quit' to exit, 'batch' to switch to batch mode\n")
...
def main():
    ...
    print("Select mode:")
    print("  [1] Batch — process sample_prompts.txt → CSV")
    print("  [2] Interactive — test prompts one by one")
    mode = input("Enter 1 or 2: ").strip()

    if mode == "1":
        prompts = load_prompts(INPUT_TEXT)
        if prompts:
